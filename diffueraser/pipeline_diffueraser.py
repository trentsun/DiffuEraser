...
class StableDiffusionDiffuEraserPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(f"[性能分析] 输入检查耗时: {time.time() - t0:.2f}s")
        ...
        print(f"[性能分析] 参数准备耗时: {time.time() - t0:.2f}s")
        ...
        print(f"[性能分析] 提示词编码耗时: {time.time() - t0:.2f}s")
        ...
        if isinstance(brushnet, BrushNetModel):
            ...
            assert False
        print(f"[性能分析] 图像预处理耗时: {time.time() - t0:.2f}s")

        # 5. Prepare timesteps
        t0 = time.time()
        timesteps, num_inference_steps = retrieve_timesteps(self.scheduler, num_inference_steps, device, timesteps)
        print(f"[性能分析] 时间步准备耗时: {time.time() - t0:.2f}s")
        ...
        print(f"[性能分析] Latents准备耗时: {time.time() - t0:.2f}s")
        ...
        print(f"[性能分析] 条件Latents准备耗时: {time.time() - t0:.2f}s")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for j, context in enumerate(context_list_choose):
                    ...
                    if (is_compiled_module(self.unet) and is_compiled_module(self.brushnet)) and is_torch_version(">=", "2.1"):
        ...
        if step_times:
            print(f"[性能分析] 平均每步耗时: {sum(step_times)/len(step_times):.2f}s")

        # 8. Post-processing
        t0 = time.time()
        video_tensor = self.decode_latents(latents, weight_dtype=prompt_embeds.dtype)
        print(f"[性能分析] 解码耗时: {time.time() - t0:.2f}s")
        ...
        print(f"[性能分析] 后处理耗时: {time.time() - t0:.2f}s")

        print(f"\n[性能分析] 总耗时: {time.time() - total_start_time:.2f}s")
        print(f"[性能分析] 峰值GPU内存: {torch.cuda.max_memory_allocated()/1024**3:.1f}GB")
